            ...
            <div key={index}>
              {typeof json[item] === "object" && !Array.isArray(json[item]) ? (
                Object.keys(json).map((subItem, index) => (
                  <div key={index} className={styles.treeDiv2}>
                    <p>{subItem}: </p>
                  </div>
                ))
                ...
                <p className={styles.treeDiv2}>{json ? `"${json}"` : "null"}</p>
              )}
            </div>
          ))
        : Array.isArray(json) ?
          json.map((item, index) => (
            <div key={index}>
              {typeof json[item] === "object" && !Array.isArray(json[item]) ? (
                ...
                <p className={styles.treeDiv2}>{ json[item] === "" ? "null" : typeof json[item] === "number" ? json[item] : typeof json === "string" && `"${json[item]}"`}</p>
              )}
            </div>
          )): <p className={styles.treeDiv2}>{ json === "" ? "null" : typeof json === "number" ? json : typeof json === "string" && `"${json}"`}</p> }
    </div>
  );
}
